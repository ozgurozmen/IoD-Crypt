...
ECCRYPTO_STATUS BPVOnline(unsigned char* lastSecret, unsigned char* lastPublic)
{
    ...
    for(j = 0; j <32; j++){
      secretTemp[j] = scalar_table[32*index[0]+j];
    }
    
    for(j = 0; j <64; j++){
      publicTemp[j] = BPV_table[64*index[0]+j];
    }
    
//    memmove(secretTemp,(void*)&scalar_table[32*index[0]],32);
//    memmove(publicTemp,(void*)&BPV_table[64*index[0]],64);
    

    
    point_setup((point_affine*)publicTemp, AddedExtproj);
    
//    memmove(secretTemp2,(void*)&scalar_table[32*index[1]],32);
//    memmove(publicTemp,(void*)&BPV_table[64*index[1]],64);

    for(j = 0; j <32; j++){
      secretTemp2[j] = scalar_table[32*index[1]+j];
    }
    
    for(j = 0; j <64; j++){
      publicTemp[j] = BPV_table[64*index[1]+j];
    }
    ...
    for(k = 0; k < BPV_k - 2; k++){
        //memmove(secretTemp,(void*)&scalar_table[32*index[k+2]],32);
        //memmove(publicTemp,(void*)&BPV_table[64*index[k+2]],64);

        for(j = 0; j <32; j++){
          secretTemp[j] = scalar_table[32*index[k+2]+j];
        }
        
        for(j = 0; j <64; j++){
          publicTemp[j] = BPV_table[64*index[k+2]+j];
        }
...
ECCRYPTO_STATUS DBPVOnline(unsigned char* lastSecret, unsigned char* lastPublic, unsigned char* lastDesignated)
{
    ...
    for(j = 0; j <64; j++){
      publicTemp2[j] = DBPV_table[64*index[0]+j];
    }
    
//    memmove(secretTemp,(void*)&scalar_table[32*index[0]],32);
//    memmove(publicTemp,(void*)&BPV_table[64*index[0]],64);
//    memmove(publicTemp2,(void*)&DBPV_table[64*index[0]],64);
    ...
    for(j = 0; j <64; j++){
      publicTemp2[j] = DBPV_table[64*index[1]+j];
    }
    
//    memmove(secretTemp2,(void*)&scalar_table[32*index[1]],32);
//    memmove(publicTemp,(void*)&BPV_table[64*index[1]],64);
//    memmove(publicTemp2,(void*)&DBPV_table[64*index[1]],64);
...
//            memmove(secretTemp,(void*)&scalar_table[32*index[k+2]],32);
//            memmove(publicTemp,(void*)&BPV_table[64*index[k+2]],64);
//            memmove(publicTemp2,(void*)&DBPV_table[64*index[k+2]],64);
        ...
        for(j = 0; j <64; j++){
          publicTemp2[j] = DBPV_table[64*index[k+2]+j];
        }
